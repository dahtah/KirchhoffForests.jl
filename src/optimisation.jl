...
function newton(G,y,t0,mu;α=0.1,numofiter = 100,tol=0.001, method="exact",nrep=100,status=true)
    t_k = copy(t0)
    tprev = copy(t0)
    increment = norm(t0)
    ...
    while( increment > tol && k < numofiter)
        ...
        if (method=="exact")
            t_k -= α*(smooth(G,q,vec_k))
        else
            vecbar = repeat(mean(vec_k, dims=1), outer=size(y,1))
            Δ = -vecbar + vec_k
            if (method=="xtilde")
                t_k -= α*(vecbar + smooth_rf(G,q,Δ,[];nrep=nrep,variant=1).est)
            elseif (method=="xbar")
                t_k -= α*(vecbar + smooth_rf(G,q,Δ,[];nrep=nrep,variant=2).est)
            end
        end
        k += 1
