...
function newton(G,y,t0,mu;α=0.05,numofiter = 100,tol=0.001, method="exact",nrep=100)
    t_k = copy(t0)
    tprev = copy(t0)
    increment = norm(t0)
    ...
    while( increment > tol && k < numofiter)
        ...
        if (method=="exact")
            ...
            Δ = -vecbar + vec_k
            if (method=="xtilde")
                t_k -= α*(vecbar + smooth_rf(G, (mu .* exp.(t_k)),Δ,[];nrep=nrep,variant=1).est)
            elseif (method=="xbar")
                t_k -= α*(vecbar + smooth_rf(G, (mu .* exp.(t_k)),Δ,[];nrep=nrep,variant=2).est)
            end
        end
        k += 1
...
x = Int64.(floor.(newton(G,y,z0,0.05;numofiter = 100,tol=0.001, method="exact")))
xtilde = Int64.(floor.(newton(G,y,z0,0.05;numofiter = 100,tol=0.001, method="xtilde",nrep=10)))
xbar = Int64.(floor.(newton(G,y,z0,0.05;numofiter = 100,tol=0.001, method="xbar",nrep=10)))
# end
Gray.(reshape(y./256.0,nx,ny))
Gray.(reshape(x./256.0,nx,ny))
Gray.(reshape(xtilde./256.0,nx,ny))
Gray.(reshape(xbar./256.0,nx,ny))

display(norm(y.-im)./norm(im))
display(norm(x.-im)./norm(im))
display(norm(xtilde.-im)./norm(im))
display(norm(xbar.-im)./norm(im))
