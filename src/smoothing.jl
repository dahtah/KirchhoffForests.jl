...
#Should update
"""
    p = Partition(rf :: RandomForest )

A simple struct for partition created by random forests. Its fields are:
-  ```part```: A vector of indices where ```part[i]``` returns the partition/tree number that node ```i``` belongs to.
-  ```nparts```: Number of different parts/trees of the partition.
-  ```sizep```: A vector of trees/parts where ```sizep[k]``` returns the size of the k-th tree.

!!! note "Local Averaging Operator"
    Local averaging operator within each tree is implemented by using this struct with multiplication. See the multiplication operator [`*`](@ref) for [`Partition`](@ref).
...
``q(q\mathsf{I}+\mathsf{L})^{-1}\mathbf{y}``, where ``\mathsf{L}`` is the graph
Laplacian and q > 0 is a regularisation coefficient (the smaller q, the stronger
the smoothing).

If Y is a matrix then this function computes
``q(q\mathsf{I}+\mathsf{L})^{-1}\mathsf{Y}``. The linear system is solved using
a direct method.
...
"""
    smooth_rf(g :: AbstractGraph,q :: Float64,Y,rootset=[];nrep=10,variant=1,mean_correction=false)
    smooth_rf(g :: AbstractGraph,q :: Vector,Y,rootset=[];nrep=10,variant=1,mean_correction=false)

This function implements RSF-based estimators for [GTR](./gtr.md) (i.e. ``\\mathsf{K}\\mathsf{Y}``) and [trace](./trace.md) of ``\\mathsf{K}``.

# Arguments
-  ```g```: Input graph.
-  ```q```: Regularization parameter. It is either a scalar or an ``n``-dimensional vector. In the latter, regularization parameter is defined node-wise.
-  ```Y```: Input graph signal. Can be an ``n``-dimensional vector. If it is a matrix, the estimators estimate ``\\mathsf{K}\\mathsf{Y}``.
-  ```rootset```: Predefined root set (Default is empty set.). The roots of the sampled forests must include ```rootset```. In terms of GTR, this setting corresponds to the limit where the regularization parameters ``q_i``s goes to infinity for the nodes in ```rootset```. In other words, no smoothing applied in these nodes.

# Optional parameters 
-  ```nrep```: Number of forests to sample.
-  ```variant```: Parameter to choose the estimator. Set ```variant=1``` for ``\\tilde{\\mathbf{x}}`` and ```variant=2``` for ``\\bar{\\mathbf{x}}``.

# Outputs:
- ```est```: Estimate for GTR problem.
- ```nroots```: Estimate for ``tr(\\mathsf{K})``.
- ```diagK```: Estimate for diagonals of ``\\mathsf{K}``.
